...
try:
    ...
    print("[!] Missing dependency: cryptography. Install it using 'pip install cryptography'")
    sys.exit(1)
import base64
...
def save_metadata(name, version, source, tags):
    meta = {
        "name": name,
        "version": version,
        "source": str(source),
...
def save(source, name, version, compress=None, encrypt=False, interactive=False, tags=None):
    source = Path(source).resolve()
    vault_path = VAULT_DIR / name
    target = get_vault_path(name, version)

    if vault_path.exists():
        if target.exists():
            confirm = input(f"[!] Vault {name}@{version} already exists. Overwrite? [y/N]: ").lower()
            if confirm != 'y':
                print("[!] Aborted.")
                return
    ...
    save_metadata(name, version, source, tags or [])
...
def vault_search(term):
    for path in VAULT_DIR.glob("**/meta.json"):
        try:
            ...
            if meta.get('name') and (term in meta['name'] or term in json.dumps(meta)):
...
Usage:
  vault save <source> <name> [--version vX] [--compress zip|tar.gz] [--encrypt] [-i] [--tags tag1 tag2]
...
def main():
    ...
    if args.command == "save":
        save(args.source, args.name, args.version, args.compress, args.encrypt, args.interactive, args.tags)
